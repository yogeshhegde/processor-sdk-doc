...
CPSW5g virtual MAC (remoteproc)
-------------------------------------------------------

Introduction
""""""""""""

The TI |__PART_FAMILY_NAME__| SoC has integrated five-port Gigabit Ethernet Switch subsystem
...
- drivers/net/ethernet/ti/j7200-cpsw-virt-mac.c
...
::
 ...
 +--------------------+ethX  +----------------------------------------+
                ...
                +-----^-------+
                      |
       +--------------v--------------------+ rpmsg_remotedev/
       | main_r5fss_cpsw5g_virt_mac0       | rpmsg_remotedev_eth_switch_ops API
       | ethernet/ti/j7200-cpsw-virt-mac.c <--------+
       +-----------------------------------+        |
                                                    |
                           +------------------------v------+
...
The j7200-cpsw-virt-mac driver is started separately by the Linux core once the corresponding main_r5fss_cpsw5g_virt_macX device has been created by the Linux DT parsing code.
Once started, the j7200-cpsw-virt-mac driver will wait for rpmsg_kdrv_switch driver to be probed.
...
except checking for the supported EthSwitch FW version. It's intended to provide the RPMSG-KDRV rpmsg_remotedev API interface to be used by the j7200-cpsw-virt-mac driver.
...
.. rubric:: **j7200-cpsw-virt-mac driver**
   :name: k3-j7200-cpsw-virt-mac-drv

The j7200-cpsw-virt-mac driver (drivers/net/ethernet/ti/j7200-cpsw-virt-mac.c) follows the standard Linux network interface architecture and supports the following features:
...
.. rubric:: **Network data flow**
   :name: k3-j7200-cpsw-virt-mac-data-flow

The EthSwitch FW is responsible for UDMA resource management and allocation between attached remote cores.
The j7200-cpsw-virt-mac driver requests the EthSwitch FW for available UDMA resources during initialization by performing an attach operation: TX channel and RX flow through the Control path (rpmsg_remotedev API).
...
Hence, after allocation, the j7200-cpsw-virt-mac driver is responsible for the full TX UDMA channel initialization (including TX and TX completion ring initialization).
The j7200-cpsw-virt-mac driver also starts/stops the TX UDMA channel.
For RX side, the j7200-cpsw-virt-mac driver only performs UDMA RX flow initialization (including RX free descriptor and RX ring initialization).
...
::
       ...
       +--------------------+ethX  +----------------------------------------+
                       ...
                       +-----^-------+
                             |
              +--------------v-------+
              | j7200-cpsw-virt-mac  |   Control Path
              |                      <----------+
              +----+---^-------------+          |
                   |   |                        |
                   |   |                        |
...
TThe EthSwitch FW needs to know which traffic to segregate to the Linux Host; therefore, the j7200-cpsw-virt-mac driver
...
Enabling j7200-cpsw-virt-mac driver
"""""""""""""""""""""""""""""""""""

The |__SDK_FULL_NAME__| package has a |__PART_FAMILY_NAME__| rpmsg_kdrv_switch and j7200-cpsw-virt-mac drivers enabled by default to be built as modules.

To enable/disable j7200-cpsw-virt-mac driver support manually, start the *Linux Kernel Configuration*
tool:
...
.. rubric:: **Device tree bindings**
   :name: k3-j7200-cpsw-virt-mac-dt-binding
...
.. rubric:: Starting the interface
   :name: k3-j7200-cpsw-virt-mac-bringing-up-interfaces
...
.. rubric:: ifconfig - Set MAC address
   :name: k3-j7200-cpsw-virt-mac-set-mac

The j7200-cpsw-virt-mac supports changing the HW MAC address, but this operation can be performed only when the network device is inactive (down).
...
.. rubric:: ethtool - Get driver information
   :name: k3-j7200-cpsw-virt-mac-ethtool-i-driver
...
.. rubric:: ethtool - Display standard information about device/link
   :name: k3-j7200-cpsw-virt-mac-ethtool-display-info
...
.. rubric:: ethtool - Show permanent hardware address
   address
   :name: k3-j7200-cpsw-virt-mac-ethtool-show-permaddr
...
.. rubric:: ethtool - RX/TX checksum offload
   :name: k3-j7200-cpsw-virt-mac-csum-offload
...
.. note::
    ...
    TX checksum offload enablement is controlled by EthSwitch FW and current status provided to j7200-cpsw-virt-mac driver.

.. rubric:: ethtool - driver testing
   :name: k3-j7200-cpsw-virt-mac-test
...
The j7200-cpsw-virt-mac can run several types of tests (if enabled by EthSwitch FW):
